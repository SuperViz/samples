...
export default async function forge({ participantId, viewerData }: Parameters ) {
  ...
  await window.Autodesk.Viewing.Initializer(options, async () => {
    viewerData.current.viewerDiv = document.getElementById(participantId) as HTMLElement;
